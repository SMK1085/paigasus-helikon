...
- **Facade reachability (refines review #9):** **no manual re-export is needed.** The facade re-exports `core` wholesale (`pub use paigasus_helikon_core as core;`) and `core` does `pub use permission::*;`, so `AllowRule` is reachable as `paigasus_helikon::core::AllowRule` the moment it is `pub` in `core`. (Review #9's call for an explicit facade `pub use` doesn't apply to this wholesale-re-export architecture — that drift risk is specific to the SMA-346 *feature-gated sibling* version pins, not to `core`.) release-plz's `dependencies_update` cascade bumps the facade version when `core` bumps via the normal flow.
- **Docs (same PR):**
