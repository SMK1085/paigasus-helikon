...
- **`GuardrailKind` gains `Serialize` + `Deserialize` derives and `Other(String)` becomes `Other { reason: String }`.** Needed for `AgentEvent::GuardrailTriggered` to round-trip. The variant-shape conversion is required because `#[serde(tag = "type")]` is incompatible with tuple variants — moving to a struct variant lets us keep the workspace-standard internally-tagged wire format (`{"type": "other", "reason": "..."}`). Pre-1.0 breaking change to a SMA-312 type with no downstream consumers; captured in §3 as a `guardrail.rs` modification.
