...
|---|---|---|---|
| rmcp version | `0.16+`, 2025-06-18 spec | **`rmcp = "1.7"`** (precise pin in `[workspace.dependencies]`) | 0.16 predates rmcp 1.0 (2026-03); eight 1.x releases since. Building new code on a year-old API buys an immediate migration ticket. |
...
| MSRV | — (workspace 1.75) | **Workspace `rust-version` → 1.85** | rmcp 1.x is edition 2024 (requires ≥ 1.85). CLAUDE.md policy: bump to what cargo demands. `ci.yml` matrix legs `1.75` → `1.85` (signal-only contexts, no ruleset change); `msrv.yml` unchanged. |
...
- `effect()` — `annotations.read_only_hint == Some(true)` → `ToolEffect::ReadOnly`,
  ...
  side-effect is the safe default).
...
- `call_tool` — parse `input`; build
  ...
  HookRegistry::new(), TracerHandle::builder().build(), CancellationToken::new())`;
...
- MSRV: workspace `rust-version` `1.75` → `1.85`; `ci.yml` matrix `1.75` →
  `1.85`; CLAUDE.md MSRV note updated. (Matrix legs are signal-only contexts —
  no ruleset edit. `msrv.yml` verifies the declared MSRV and needs no change.)
...
- `ToolSource` trait in core + `.mcp_servers(...)` builder sugar.
