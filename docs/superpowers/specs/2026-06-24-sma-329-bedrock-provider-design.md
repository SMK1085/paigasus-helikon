...
## 0. ⚠️ MSRV — RESOLVED (raise workspace MSRV to 1.94)

> **RESOLVED (GATE 1, 2026-06-24): option (A) — raise the workspace MSRV.** Landed as a clearly-scoped `chore` commit within this PR (root `Cargo.toml` `rust-version`, `ci.yml` `test` matrix MSRV row, `msrv.yml`, README/CLAUDE.md MSRV text). Also at GATE 1: **R1 release path = manual name-claim pre-publish** of `bedrock 0.1.0` (§11/R1).
>
> **AMENDED (2026-06-25): target MSRV is 1.94, not 1.91.** Task 1 of implementation discovered that `sqlx 0.9.0` — **already in `main`'s `Cargo.lock`**, used by `paigasus-helikon-sessions-sqlite` — declares `rust-version = 1.94.0`, and cargo's MSRV resolver refuses to build it on 1.91 (verified: `cargo +1.91 build -p paigasus-helikon-sessions-sqlite --all-features` → "sqlx@0.9.0 requires rustc 1.94.0"). So the workspace's real floor is already **1.94** today (the declared `1.85` only survives because `test (…, 1.85)` is signal-only and `msrv.yml` checks core, which has no sqlx). AWS adds a 1.91 floor; combined floor = **max(1.94, 1.91) = 1.94**. Workspace MSRV is therefore set to **1.94** (CI `test` matrix MSRV row = `1.94`).
...
- **TLS / licenses / deny (challenge MAJOR, R3):** configure the AWS SDK onto **aws-lc-rs** (matching the workspace's existing `aws-lc-rs` usage via `reqwest`/`async-openai`) — the specific `aws-config`/`aws-smithy-runtime` `aws-lc-rs`-backed rustls features + `default-features = false` dance is pinned in the plan. Then run `cargo deny check licenses` against the **resolved aws-lc-rs-configured** graph and enumerate any licenses needing allowlisting in `deny.toml`, **each with justification** (matching the existing comment style), across all **five** `deny.toml` targets. This is an **early-plan gate**, not a post-hoc fixup.
...
- **R2 — MSRV 1.91 vs 1.85 (BLOCKER; see §0).** Verified: `aws-sdk-bedrockruntime` 1.135.0 and `aws-config` 1.8.18 require **1.91.1**; the highest `aws-sdk-bedrockruntime` with MSRV ≤ 1.85 is ~1.86.0 (MSRV 1.81, May 2025). **Decision: raise the workspace MSRV to 1.94** (scoped `chore` commit in this PR). *Amended 2026-06-25 from 1.91 → 1.94:* the AWS floor is 1.91, but `sqlx 0.9.0` (pre-existing on `main`, used by `sessions-sqlite`) declares MSRV **1.94.0** and cargo's resolver refuses to build it below 1.94 — verified with the CI-style command `cargo +1.91 build -p paigasus-helikon-sessions-sqlite --all-features`. The combined real floor is **1.94**; setting it also corrects a latent understatement on `main` (declared 1.85 < real 1.94). The plan verifies with `cargo +1.94 build --workspace --all-features` and updates the `ci.yml` `test` matrix MSRV row, `msrv.yml`, and README/CLAUDE.md MSRV statements in the same PR.
...
- **R3 — TLS provider / licenses — RESOLVED (2026-06-25), and the original guidance was backwards.** The original plan said "configure the AWS SDK onto rustls/ring." Task 1 discovered that is wrong for this workspace: it **already standardizes on `aws-lc-rs`** as its rustls `CryptoProvider` (`reqwest` → `async-openai` pull `aws-lc-rs`/`aws-lc-sys`, both already in `main`'s `Cargo.lock` and already deny-allowed). Forcing the AWS SDK onto a *ring*-based rustls introduces a **second** provider → a "multiple providers" runtime panic (hit in the `forkd_tls` tests). **Decision: configure the AWS SDK's HTTPS connector on `aws-lc-rs`** — the same provider the workspace already uses. Consequences: one `CryptoProvider`, no panic, and **no new licenses** (aws-lc is already allowed, so `cargo deny check` stays green without `deny.toml` changes). The bedrock crate must explicitly wire this connector (the scaffold initially left it unwired).
