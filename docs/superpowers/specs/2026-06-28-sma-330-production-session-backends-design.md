...
5. **Guard — skip only a degenerate snapshot:** if `snap.messages.is_empty()` **or** (`snap.messages.len() == 1` **and** that message is `Item::System`) → return. A lone running `System` summary (or an empty snapshot) has nothing useful to collapse; keying on the projected *messages* (not the raw event count) avoids the `[Compacted, handoff]` ping-pong. **Note:** a plain `messages.len() <= 1` is too aggressive — a single non-summary message that is itself over threshold *should* compact, so the guard must check the message is a `System` summary, not merely that there is one message.
