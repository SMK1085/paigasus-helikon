  ...
  agentcore-image:
    ...
    steps:
      ...
      - id: decide
        ...
        run: |
          # No nightly: the images build in ~4 minutes cold (measured in CI),
          # so this isn't a cost call. The numbers sit far under their budgets
          # (~11% of the size gate, ~20% of the latency gate) and cannot drift
          # without a first-party change the path filter above already catches
          # on the next PR — re-measuring two stable numbers every night adds
          # no information. workflow_dispatch still reaches this job.
