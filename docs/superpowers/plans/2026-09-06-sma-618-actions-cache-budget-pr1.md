    ...
    steps:
      - name: Report Actions cache usage
        ...
        run: |
          ...
          rows="$(gh api --paginate "repos/${GITHUB_REPOSITORY}/actions/caches" \
          ...
          if [ "${total}" -gt "${WARN_BYTES}" ]; then
            echo "::warning::Actions cache usage is ${human_total}, above the $(numfmt --to=iec --suffix=B "${WARN_BYTES}") warning line. GitHub's hard limit is 10 GB and eviction is silent. See docs/runbooks/ci-architecture.md -> Actions cache budget."
          fi
```
