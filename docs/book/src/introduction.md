...
- **Runtime.** `paigasus_helikon::runtime_tokio` (feature `runtime-tokio`) is the default ephemeral Tokio runner. `paigasus_helikon::runtime_axum` (feature `runtime-axum`) is the self-hosted HTTP/SSE/WebSocket agent server built on axum — see [Axum Server Runtime](concepts/axum-server.md). `paigasus_helikon::runtime_actix` (feature `runtime-actix`) is the same self-hosted HTTP/SSE/WebSocket agent server built on actix-web instead of axum. `paigasus_helikon::runtime_temporal` (feature `runtime-temporal`) is a durable runner backed by the Temporal Rust SDK, and `paigasus_helikon::runtime_agentcore` (feature `runtime-agentcore`) is an AWS Bedrock AgentCore container shim — see [Runtimes](concepts/runtimes.md) for both.
