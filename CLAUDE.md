...
**Released vs stub crates** (exact versions move every release — read each crate's `Cargo.toml` for current numbers, don't trust hardcoded values here): seven crates publish normally (`paigasus-helikon-core`, the `paigasus-helikon` facade, `-macros`, `-providers-openai`, `-providers-anthropic`, `-sessions-sqlite`, and `-runtime-tokio` — which ascended from stub in SMA-346). The six remaining stubs (`-mcp`, `-tools`, `-evals`, `-runtime-axum`, `-runtime-temporal`, `-runtime-agentcore`) stay at `0.0.0` with `publish = false` + a `release = false` block in `release-plz.toml`. `paigasus-helikon-cli` is binary-only and never published.
