...
**Released vs stub crates** (exact versions move every release — read each crate's `Cargo.toml` for current numbers, don't trust hardcoded values here): nine crates publish normally (`paigasus-helikon-core`, the `paigasus-helikon` facade, `-macros`, `-providers-openai`, `-providers-anthropic`, `-sessions-sqlite`, `-runtime-tokio` (ascended from stub in SMA-346), `-mcp` (SMA-327), and `-tools` (SMA-328)). The four remaining stubs (`-evals`, `-runtime-axum`, `-runtime-temporal`, `-runtime-agentcore`) stay at `0.0.0` with `publish = false` + a `release = false` block in `release-plz.toml`. `paigasus-helikon-cli` is binary-only and never published.
...
The required-status-check contexts gated on `main` are (bare job names, as posted by the GitHub Actions app): `fmt`, `clippy`, `test (ubuntu-latest, stable)`, `test (macos-latest, stable)`, `docs`, `doc-coverage`, `commits`, `pr-title`, `audit`, `deny`. The macOS job is required because it is the only gate that compiles and runs the Seatbelt backend. The canonical declaration is `.github/rulesets/main-protection-checks.json` (see CONTRIBUTING.md → "Repo configuration"). Other matrix variants (`test (windows-latest, …)`, `test (…, 1.85)`) run as signals only. Concurrency cancels in-flight PR runs but lets `main` pushes complete; both workflows declare `permissions: contents: read`.
