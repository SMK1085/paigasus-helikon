...
| `.github/rulesets/main-protection-checks.json` | Required status checks, linear history, no force-push, no deletion. Enforced on admins (no bypass). Required contexts: `fmt`, `clippy`, `test (ubuntu-latest, stable)`, `test (macos-latest, stable)` (required because it is the only gate that compiles and runs the Seatbelt backend), `docs`, `doc-coverage`, `book-build`, `commits`, `pr-title`, `audit`, `deny`, `sessions-it` (required because it is the only gate that runs the Postgres/Redis session backends against live servers). |
