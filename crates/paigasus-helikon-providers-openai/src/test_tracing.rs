...
//! scheduling.
//!
//! That is not hypothetical. SMA-619 added such a test to `backend/chat.rs`
//! and an identical one to `providers-litellm`'s `stream.rs`; the litellm
//! twin was measured at **52 failures in 200 runs** of that crate's lib
//! suite and turned `test (macos-latest, stable)` red in CI. This crate's
//! copy never failed under measurement — it has fewer sibling tests
//! reaching the same callsite unsubscribed, so it wins the race more often
//! — which is exactly why it is not left as it was. Returning
//! `Interest::sometimes()` from `register_callsite` and calling
//! `rebuild_interest_cache()` only narrowed the litellm rate to 35 in 200 —
//! the rebuild races the sibling tests rather than beating them.
//!
